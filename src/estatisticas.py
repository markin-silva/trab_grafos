...
def floyd_warshall_misto(grafo):
    """Todos-os-pares para grafo com arestas (dupla) e arcos (direcionais)."""
    nos = list(grafo.nos.keys())

    # inicialização
    dist = {u: {v: float("inf") for v in nos} for u in nos}
    pred = {u: {v: None            for v in nos} for u in nos}
    for v in nos:
        dist[v][v] = 0

    # arestas bidirecionais
    for e in grafo.arestas:
        u, v, c = e["de"], e["para"], e["custo"]
        if c < dist[u][v]:
            dist[u][v] = dist[v][u] = c
            pred[u][v], pred[v][u] = u, v

    # arcos dirigidos (só a direção existente)
    for a in grafo.arcos:
        u, v, c = a["de"], a["para"], a["custo"]
        if c < dist[u][v]:
            dist[u][v] = c
            pred[u][v] = u
        # se permitir trafegar “de ré”, descomente:
